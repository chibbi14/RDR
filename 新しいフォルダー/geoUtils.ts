 ...
 */
export const computeMVASectors = (rings: number[], radials: any[], arcs: any[], labels: any[]) => {
  ...
  return labels.map((l, idx) => {
    ...
    const relevantArcs = (arcs || []).filter(a => a.isFull || isAngleBetween(deg, a.start, a.end));
    const r1 = relevantArcs.filter(a => a.r <= r).reduce((max, a) => Math.max(max, a.r), 0);
    const r2 = relevantArcs.filter(a => a.r > r).reduce((min, a) => Math.min(min, a.r), 50);
